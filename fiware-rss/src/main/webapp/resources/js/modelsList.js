    ...
    var fillModels = function (models) {
        for (var i = 0; i < models.length; i++) {
            ...
            if(models[i].stakeholders.length > 0) {
                var stTitle = '<tr><th colspan="3">Stakeholder</th><th colspan="3">Stakeholder Value</th></tr>';
                table.append(stTitle);
                for (var j = 0; j < models[i].stakeholders.length; j++) {
                    var row = $('<tr></tr>');
                    row.append('<td colspan="3">' + models[i].stakeholders[j].stakeholderId + '</td>');
                    row.append('<td colspan="3">' + models[i].stakeholders[j].modelValue + '</td>');
                    table.append(row);
                }
            }
            table.appendTo('#models-container');
        }
    };
